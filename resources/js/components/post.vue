...
<div class="btn-group dropleft single-menu">
  ...
  <div class="dropdown-menu">
    ...
    <button class="dropdown-item" type="button">Report</button>
        ...
        <div v-if="isMobile" class="fullscreen-view__scrollable" v-bind:style="{ transform:'translateY('+x+')' }" >
            <!---Previous block---->
                <div class="fullscreen-view__entry-container" v-for="(post,index) in posts" v-bind:key="index" :title="index"  >
  ...
  methods:{
   touchstart(event) {
      ...
      touchmove(event) {
        ...
        {
          var diff=this.touch.startY-this.touch.endY;
          var m=this.b+diff;
          this.x="-"+m+"px";
        }else{
          var diff=this.touch.startY-this.touch.endY
      ...
      touchend() {
        if(!this.touch.endY || Math.abs(this.touch.endY - this.touch.startY) < 20)
          return;
          //console.log(this.touch.endY);
          //console.log(this.touch.startY);
          //console.log(this.a);
          var diff=this.touch.startY-this.touch.endY;
          var dint=Math.abs(diff);
        if(this.touch.endY < this.touch.startY)
        {
          if(dint>100){
          if(this.hasNext)
          {
            ...
            var j=this.x;
            this.mobilenext();
            var y=this.screenheight*this.a;
            this.x="-"+y+"px";
            this.b=y;
            }else{
              var y=this.screenheight*this.a;
    ...
    var that=this;
            ...
            that.timee=setInterval(function(){
              if(that.hasNext){
                ...
                var j=that.x;
                that.mobilenext();
                var y=that.screenheight*that.a;
                that.x="-"+y+"px";
                that.b=y;
              }else{
